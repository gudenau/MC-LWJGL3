...
import net.gudenau.lwjgl3.helper.KeyboardHelper;
...
public class Keyboard{
    ...
    private static KeyEvent latestEvent;
    public static final int KEYBOARD_SIZE = 256;
    private static String[] keyNames = new String[KEYBOARD_SIZE];
    private static Map<String, Integer> keyMap = new HashMap<>();
    private static boolean enableRepeats;
    ...
    private static void keyCallback(long window, int key, int scancode, int action, int mods){
        if(Keyboard.window == window){
            synchronized(eventQueue$lock){
                eventQueue.add(new KeyEvent(key, scancode, action, mods));
            }
        }
    }

    private static void charCallback(long window, int character){
        if(Keyboard.window == window){
            synchronized(eventQueue$lock){
                //eventQueue.add(new KeyEvent(character));
            }
        }
    }
    ...
    private static class KeyEvent{
        ...
        KeyEvent(int key, int scancode, int action, int mods){
            this.key = KeyboardHelper.translateKeyFromGLFW(key);
            this.scancode = scancode;
            this.action = action;
            this.mods = mods;
            character = KeyboardHelper.getCharacter(this.key, mods);
        }
    ...
    public static void create() throws LWJGLException{
        keyCallback = GLFWKeyCallback.create(Keyboard::keyCallback);
        //charCallback = GLFWCharCallback.create(Keyboard::charCallback);
    }

    public static boolean isCreated(){
        return keyCallback != null;
    }
    ...
    public static void poll(){
        synchronized(eventQueue$lock){
            if(eventQueue.isEmpty()){
                ...
                latestEvent = ((List<KeyEvent>)eventQueue).get(eventQueue.size() - 1);
                eventQueue.clear();
            }
        }
    }
    ...
    public static boolean isKeyDown(int key){
        int glfwKey = KeyboardHelper.translateKeyToGLFW(key);
        return glfwKey != -1 && window != NULL && glfwGetKey(window, glfwKey) == GLFW_PRESS;
    }
    ...
    public static void enableRepeatEvents(boolean enable){
        enableRepeats = enable;
    }

    public static boolean areRepeatEventsEnabled(){
        return enableRepeats;
    }
    ...
    public static char getEventCharacter(){
        return (char)(latestEvent == null ? 0 : latestEvent.getCharacter());
    }

    public static int getEventKey(){
        return latestEvent == null ? 0 : latestEvent.getKey();
    }
    ...
    public static boolean getEventKeyState(){
        ...
        int action = latestEvent.getAction();
    ...
    public static boolean isRepeatEvent(){
        return latestEvent != null && latestEvent.getAction() == GLFW_REPEAT;
    }
